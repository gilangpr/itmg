...
class Newscategory_RequestController extends MyIndo_Controller_Action
{
	protected $_model;
	protected $_limit;
	protected $_start;
	...
	public function init()
	{
		...
		$this->_model = new Application_Model_NewsCategory();
		...
		$this->_pk = 'NEWS_CATEGORY_ID';
		$this->_name = 'NEWS_CATEGORY';
	}
				...
				try {
					...
					if(isset($this->_posts['query'])) {
						if(!empty($this->_posts['query']) && $this->_posts['query'] != '') {
							$q->where('NEWS_CATEGORY LIKE ?', '%' . $this->_posts['query'] . '%');
						}
					}
